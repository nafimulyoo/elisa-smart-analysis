...
def test_basic_knowledge(question):
    ...
    result = asyncio.run(ask_about_elisa.run(question))
...
def test_unrelevant(question):
    respond_initial_prompt = RespondInitialPrompt()
    result = asyncio.run(respond_initial_prompt.run(question))
...
def test_data_availability(question):
    ...
    result = asyncio.run(ask_data_availability.run(question[0]))
