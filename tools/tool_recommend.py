...
class ToolRecommender(BaseModel):
    ...
    async def rank_tools(
        ...
        if current_task:
            # if there is "## Example" and "## Your Current Task", inside current_tas remove string between it
            if ("## Example" in current_task) and ("## Your Current Task" in current_task) :
                current_task_1 = current_task.split("## Example")[0]
                current_task_2 = current_task.split("## Your Current Task")[1]

                current_task = current_task_1 + current_task_2
            if "## Your Answer in this format" in current_task:
                current_task = current_task.split("## Your Answer in this format")[0]
        ...
        # logger.warning("LLM TOOLS RANK PROMPT: \n" + prompt)
        rsp = await LLM().aask(prompt)
        rsp = CodeParser.parse_code(block=None, text=rsp)
        # logger.warning("LLM TOOLS RANK response: \n" + rsp)
